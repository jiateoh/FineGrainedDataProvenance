...
/**
  ...
  */
class WrappedPairRDD[K,V](rdd: PairRDDFunctions[AnyTracker[K],AnyTracker[V]]) (implicit kt: ClassTag[K], vt: ClassTag[V], ord: Ordering[K] = null)
extends Serializable
{
  ...
  def reduceByKey(func: (V, V) => V): WrappedRDD[(K, V)] = return new WrappedRDD[(K,V)](rdd.reduceByKey{ (v1, v2) =>
    val value = func(v1.value, v2.value)
    rankBitmaps(v1.bitmap , v2.bitmap)
    new AnyTracker(value,v1.bitmap)
  })
