...
import examples.benchmarks.bigsift.interfaces.{Splitting, Testing}
...
class DDNonExhaustive[T: ClassTag] {
    ...
    breakable {
      ...
      while (!failing_stack.isEmpty) {
        breakable {
          ...
          printlog(logger, sizeRdd, runTime,startTime, subrdd.rdd)
          ...
          if(runTime == 1) {
            ...
            runTime = runTime + 1
            printlog(logger, sizeRdd, runTime, startTime, rdd);
          }
          ...
          while(!rdd_failed && i<partitions){
            ...
            runTime = runTime + 1
            printlog(logger, sizeRdd, runTime , startTime, rddList(i));
            if (i == 0) {
              first_rdd_runTime = first_rdd_runTime + 1
            }
            else {
          ...
          var j = 0;
            while(!rdd_failed && j<partitions){
              ...
              runTime = runTime + 1
              printlog(logger, sizeRdd, runTime , startTime, rddBar);
              if (result) {
                rddBar_failed = true
                rdd_failed = true
                next_rdd = rddBar
  ...
  def printlog( logger : Logger, sizeRdd: Long , runTime : Int, startTime : Long , rdd: Array[T]): Unit ={
    val endTime = System.nanoTime
    bus.postToAll(FaultLocalizationInfo((endTime - startTime) / 1000,sizeRdd, 0 ,rdd.take(10).map(s=>s.toString)))
  ...
  def printlog( logger : Logger, sizeRdd: Long , runTime : Int, startTime : Long , rdd: RDD[T]): Unit ={
    val endTime = System.nanoTime
    try {
      //Thread.sleep(250)
      bus.postToAll(FaultLocalizationInfo((endTime - startTime) / 1000, sizeRdd, 0, rdd.take(10).map(s => s.toString)))
