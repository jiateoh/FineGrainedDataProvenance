...
trait PairProvenanceRDD[K, V] extends ProvenanceRDD[(K, V)] {
  ...
  def mapValues[U:ClassTag](f: V => U,enableUDFAwareProv: Boolean = true): PairProvenanceRDD[K, U]
 ...
 def combineByKeyWithClassTag[C](
                                     ...
                                     inflFunctionCV:Option[(C,V) => InfluenceMarker] = None)
  ...
  def reduceByKey(func: (V, V) => V , influence:(V,V) => InfluenceMarker): PairProvenanceRDD[K, V] = {
    combineByKeyWithClassTag(identity, func, func, inflFunctionCC=Some(influence), inflFunctionCV=Some(influence))(vct)
  }
